...
const AssignNurseModal = ({ isOpen, onClose, sessionId, preselectedClassId = null, onNurseAssigned = null }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      await Promise.all([fetchNurseList(), fetchSessionClasses()]);
    } catch (err) {
  ...
  const fetchSessionClasses = async () => {
    ...
    const availableClasses = validClasses.filter(cls => !assignedClassIds.includes(cls.id));
    setAvailableClasses(availableClasses);

    if (preselectedClassId && availableClasses.find(cls => cls.id === preselectedClassId)) {
      setAssignments([{ id: 1, nurseId: "", classIds: [preselectedClassId] }]);
  ...
  const handleAssignNurse = async () => {
    ...
    try {
      ...
      const res = await vaccineSessionApi.assignNurseToVaccinationSession({
        sessionId,
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: TEXT.SECONDARY }}>
                Chọn nhân viên y tế cho các lớp chưa được phân công
              </p>
            </div>
          </div>
          <button
              ...
              <div className="space-y-4">
                {assignments.length === 0 && availableClasses.length === 0 ? (
                  <div className="text-center py-8" style={{ color: GRAY[500] }}>
                    ...
                    <p className="text-sm">Không có lớp nào cần phân công thêm</p>
                  </div>
                ) : (
                  <>
                      ...
                      {assignments.map((assignment, index) => (
                        <div key={assignment.id} className="p-4 rounded-lg border" style={{ borderColor: BORDER.DEFAULT }}>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                              <label className="block text-sm font-medium mb-2" style={{ color: TEXT.PRIMARY }}>
                                Lớp được phân công ({assignment.classIds.length})
                              </label>
                                ...
                                {getAvailableClassesForAssignment(assignment.id).length === 0 ? (
                                  ...
                                  <div className="grid grid-cols-1 gap-1">
                                    {getAvailableClassesForAssignment(assignment.id).map((cls) => (
                                      <label key={cls.id} className="flex items-center gap-2 text-sm p-1 rounded hover:bg-gray-50">
                                        <input
                                          type="checkbox"
                                          ...
                                          className="rounded"
                                        />
                                        <span style={{ color: TEXT.PRIMARY }}>{cls.name}</span>
                                      </label>
                                    ))}
                                  </div>
                                )}
                    ...
                    <div className="flex justify-center pt-2">
                      <button
                        ...
                        style={{
                          backgroundColor: hasAvailableClasses ? PRIMARY[50] : GRAY[100],
                          color: hasAvailableClasses ? PRIMARY[600] : GRAY[500],
                          border: `1px solid ${hasAvailableClasses ? PRIMARY[200] : GRAY[300]}`
                        }}
                      >
