...
const ViewAllVisionModal = ({ isOpen, onClose, visionRecords = [] }) => {
    if (!isOpen) return null;

    const sortedVisionRecords = Array.isArray(visionRecords) 
        ? [...visionRecords].sort((a, b) => new Date(b.checkDate) - new Date(a.checkDate))
        : [];
                ...
                <div className="flex items-center justify-between p-6 border-b"
                    ...
                    <div className="flex items-center">
                        <div className="w-12 h-12 rounded-full flex items-center justify-center mr-4"
                            ...
                            <FiEye className="h-6 w-6" style={{ color: PRIMARY[600] }} />
                        </div>
                        <div>
                            <h2 className="text-2xl font-bold" style={{ color: TEXT.PRIMARY }}>
                                Tất cả lịch sử thị lực
                            </h2>
                            <p className="text-sm" style={{ color: TEXT.SECONDARY }}>
                                Tổng cộng {sortedVisionRecords.length} lần kiểm tra
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
                    {sortedVisionRecords.length === 0 ? (
                        <div className="text-center py-8" style={{ color: TEXT.SECONDARY }}>
                            <FiEye className="h-12 w-12 mx-auto mb-4 opacity-50" />
                            <p>Chưa có lịch sử kiểm tra thị lực</p>
                        </div>
                    ) : (
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y" style={{ borderColor: GRAY[200] }}>
                                ...
                                <tbody className="bg-white divide-y" style={{ borderColor: GRAY[200] }}>
                                    {sortedVisionRecords.map((record, index) => (
                                        <tr key={index} className="hover:bg-gray-50 transition-colors duration-200">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-bold" style={{ color: TEXT.PRIMARY }}>
                                                {record.leftEye > 0 && record.leftEye !== 'Not recorded' 
                                                    ? `${record.leftEye}/10` 
                                                    : 'Không có dữ liệu'
                                                }
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-bold" style={{ color: TEXT.PRIMARY }}>
                                                {record.rightEye > 0 && record.rightEye !== 'Not recorded' 
                                                    ? `${record.rightEye}/10` 
                                                    : 'Không có dữ liệu'
                                                }
                                            </td>
...
export default ViewAllVisionModal; 