...
const HealthCheckSchedule = () => {
    ...
    const [healthCheckPlans, setHealthCheckPlans] = useState([]);
    ...
    const fetchStudents = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setSelectedStudentData(response.data[0] || null);
            } else {
    ...
    const fetchHealthCheckPlans = async () => {
        ...
        try {
            ...
            const response = await healthCheckApi.getStudentHealthCheckPlans(selectedStudentData.id, params);
            if (response.success) {
                setHealthCheckPlans(response.data || []);
    ...
    const filteredPlans = useMemo(() => {
        let filteredData = healthCheckPlans.filter(item => {
            const statusMap = {
                'waiting': ['waiting', 'waitingforparentconsent', 'waitingforparentconsent'],
                'scheduled': ['scheduled', 'inprogress'],
                'completed': ['completed', 'done']
            };
            const statusFilter = statusMap[activeTab];
            const matchesStatus = !statusFilter || statusFilter.includes(item.status?.toLowerCase());
        ...
        return filteredData;
    }, [healthCheckPlans, activeTab, debouncedSearchTerm]);
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: BACKGROUND.NEUTRAL }}>
                <Loading type="medical" size="large" color="primary" text="Đang tải lịch khám sức khỏe..." />
            </div>
        );
    }
                ...
                <div className="container mx-auto px-4 relative z-10">
                    <div className="text-center text-white max-w-5xl mx-auto">
                        ...
                        <p className="text-base sm:text-lg opacity-90 leading-relaxed font-medium px-4">
                            Quản lý lịch khám sức khỏe cho con em
                        </p>
                    </div>
                </div>
            </section>
            ...
            <div className="px-4 sm:px-6 lg:px-8 py-8">
                <div className="rounded-2xl shadow-xl border backdrop-blur-sm" style={{ backgroundColor: 'rgba(255, 255, 255, 0.95)', borderColor: GRAY[200] }}>
                        ...
                        <div className="flex flex-col lg:flex-row gap-4 lg:items-center">
                            ...
                            <div className="flex-1">
                                <div className="relative">
                                    ...
                                    <input
                                        type="text"
                                        placeholder="Tìm kiếm theo tên kế hoạch, địa điểm..."
                    ...
                    <div className="overflow-hidden relative">
                        {isSearching || loadingPlans ? (
                            <div className="text-center py-16">
                                <Loading type="medical" size="large" color="primary" text={isSearching ? "Đang tìm kiếm lịch khám sức khỏe..." : "Đang tải lịch khám sức khỏe..."} />
                            </div>
                            ...
                            <table className="w-full table-fixed">
                                ...
                                <tbody className="divide-y" style={{ divideColor: GRAY[100] }}>
                                    {filteredPlans.map((plan, index) => (
                                        <tr key={plan.id} className="hover:bg-gray-50 transition-all duration-200 group"
                                            ...
                                            <td className="py-4 px-6 w-1/4">
                                                <div className="flex flex-col">
                                                    <span className="font-semibold" style={{ color: TEXT.PRIMARY }}>
                                                        {plan.title || "Kế hoạch khám sức khỏe"}
                                                    </span>
                                            ...
                                            <td className="py-4 px-6 w-1/6">
                                                <div className="flex flex-col gap-1">
                                                    <div className="text-sm font-medium" style={{ color: TEXT.PRIMARY }}>
                                                        {plan.startTime ? new Date(plan.startTime).toLocaleTimeString("vi-VN", { hour: '2-digit', minute: '2-digit' }) : '--'}
                                                        <span className="text-sm font-medium" style={{ color: TEXT.PRIMARY }}>
                                                            {plan.endTime ? ` - ${new Date(plan.endTime).toLocaleTimeString("vi-VN", { hour: '2-digit', minute: '2-digit' })}` : ''}
                                                        </span>
                                                    </div>
                                                    <div
                                                        ...
                                                        style={{
                                                            backgroundColor: (() => {
                                                                const startDate = plan.startTime ? new Date(plan.startTime) : null;
                                                                const now = new Date();
                                                                if (!startDate) return PRIMARY[100];
                                                            ...
                                                            color: (() => {
                                                                const startDate = plan.startTime ? new Date(plan.startTime) : null;
                                                                const now = new Date();
                                                                if (!startDate) return PRIMARY[700];
                                                                const dueIn = Math.ceil((startDate - now) / (1000 * 60 * 60 * 24));
                                            ...
                                            <td className="py-4 px-6 w-1/6">
                                                <span className="text-sm font-medium" style={{ color: TEXT.PRIMARY }}>
                                                    {plan.responsibleOrganizationName || '--'}
                                                </span>
                                            </td>
                                            <td className="py-4 px-6 w-1/6">
                            ...
                            <div className="text-center py-16 lg:py-24">
                                <div className="max-w-md mx-auto">
                                    ...
                                    <h3 className="text-xl lg:text-2xl font-bold mb-4" style={{ color: TEXT.PRIMARY }}>
                                        Không tìm thấy lịch khám sức khỏe nào
                                    </h3>
