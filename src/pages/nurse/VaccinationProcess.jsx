...
const VaccinationProcess = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const assignedClasses = session.classNurseAssignments?.filter(
                assignment => assignment.nurseId === user?.id
            ) || [];
            ...
            const validData = consentResponse
    ...
    const filteredStudents = students
        ...
        .sort((a, b) => {
            const statusOrder = { "InProgress": 1, "Completed": 2, "NotVaccinated": 3 };
            return statusOrder[a.vaccinationStatus] - statusOrder[b.vaccinationStatus];
        });
            ...
            setStudents(prevStudents =>
                prevStudents.map(student =>
                    student.studentId === selectedStudent.studentId
                        ? { ...student, vaccinationStatus: vaccinationData.vaccinationStatus }
                        : student
                )
            );
    ...
    const StatusBadge = ({ status, type }) => {
        ...
        const badgeConfig = config[type][status] || config[type].default;
        return (
            <div
                                    ...
                                    <div className="space-y-2">
                                        {filteredStudents.map((student) => (
                                            <div
                                                ...
                                                style={{
                                                    backgroundColor: selectedStudent?.studentId === student.studentId ? `${PRIMARY[50]}` : BACKGROUND.DEFAULT,
                                                    borderColor: selectedStudent?.studentId === student.studentId ? PRIMARY[400] : BORDER.DEFAULT
                                                }}
                                            >
                                                ...
                                                <div className="flex items-center space-x-4">
                                                    <div className="flex-shrink-0">
                                                        <div
                                                            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-200 ${selectedStudent?.studentId === student.studentId ? 'ring-2 ring-primary-300' : 'group-hover:ring-2 group-hover:ring-primary-200'}`}
                                                            style={{ backgroundColor: selectedStudent?.studentId === student.studentId ? PRIMARY[200] : PRIMARY[100] }}
                                                        >
                                                            <FiUser
                                                                className={`w-6 h-6 transition-all duration-200 ${selectedStudent?.studentId === student.studentId ? 'scale-110' : 'group-hover:scale-105'}`}
                                                                style={{ color: PRIMARY[600] }}
                                                            />
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center justify-between mb-1">
                                                            <h3 className={`font-semibold truncate transition-all duration-200 ${selectedStudent?.studentId === student.studentId ? 'text-lg' : 'text-base'}`}
                        ...
                        <div className="p-6">
                            {selectedStudent ? (
                                (selectedStudent.vaccinationStatus === "Completed" || selectedStudent.vaccinationStatus === "NotVaccinated") ? (
                                    <VaccinationResult student={selectedStudent} sessionId={id} onEdit={handleVaccinationSubmit} />
                                ) : (
                                    <VaccinationForm student={selectedStudent} onSubmit={handleVaccinationSubmit} sessionId={id} />
                                )
                            ) : (
                                <div className="text-center py-12">
...
const VaccinationResult = ({ student, sessionId, onEdit }) => {
    ...
    const handleEdit = async (e) => {
        e.preventDefault();
        setError(null);
        if (isCompleted && !formData.symptoms) return setError("Vui lòng nhập triệu chứng.")
        if (!isCompleted && !formData.reason) return setError("Vui lòng nhập lý do không tiêm.")
        ...
        try {
            ...
            if (response.success) {
                await onEdit({
                    vaccinationStatus: isCompleted ? "Completed" : "NotVaccinated",
                    ...data
                });
                setIsEditing(false);
            } else {
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold" style={{
                            color: isCompleted ? PRIMARY[700] : GRAY[700]
                        }}>
                            {student.studentName}
                        </h3>
                        <p className="text-sm flex items-center" style={{
                            color: isCompleted ? PRIMARY[600] : GRAY[600]
                        }}>
                            Lớp {student.className}
                        </p>
                        <div className="mt-1">
            ...
            {!isEditing ? (
                <div className="space-y-6">
                    <div>
                        <h4 className="text-lg font-semibold mb-4" style={{ color: TEXT.PRIMARY }}>
                            {isCompleted ? "Kết quả tiêm chủng" : "Thông tin không tiêm chủng"}
                        </h4>
                        <div className="space-y-4">
                            {isCompleted ? (
                                <div className="p-4 rounded-xl border" style={{ backgroundColor: BACKGROUND.DEFAULT, borderColor: BORDER.DEFAULT }}>
                                    <h5 className="font-medium mb-2" style={{ color: TEXT.SECONDARY }}>Triệu chứng sau tiêm:</h5>
                                    <p style={{ color: TEXT.PRIMARY }}>{student.symptoms || "Không có triệu chứng"}</p>
                                </div>
                            ) : (
                                <div className="p-4 rounded-xl border" style={{ backgroundColor: BACKGROUND.DEFAULT, borderColor: BORDER.DEFAULT }}>
                                    <h5 className="font-medium mb-2" style={{ color: TEXT.SECONDARY }}>Lý do không tiêm:</h5>
                                    <p style={{ color: TEXT.PRIMARY }}>{student.reason || "Không có lý do"}</p>
                                </div>
                            )}

                            {student.noteAfterSession && (
                                <div className="p-4 rounded-xl border" style={{ backgroundColor: BACKGROUND.DEFAULT, borderColor: BORDER.DEFAULT }}>
                                    <h5 className="font-medium mb-2" style={{ color: TEXT.SECONDARY }}>Ghi chú:</h5>
                                    <p style={{ color: TEXT.PRIMARY }}>{student.noteAfterSession}</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    <h4 className="text-lg font-semibold mb-4" style={{ color: TEXT.PRIMARY }}>
                        Chỉnh sửa thông tin {isCompleted ? "tiêm chủng" : "không tiêm chủng"}
                    </h4>
...
const VaccinationForm = ({ student, onSubmit, sessionId }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.success) {
                await onSubmit({
                    vaccinationStatus: selectedOption === "vaccinated" ? "Completed" : "NotVaccinated",
                    ...data
                });
                setFormData({ symptoms: '', noteAfterSession: '', reason: '' });
                setSelectedOption("vaccinated");
            } else {
                setError(response.message || "Có lỗi xảy ra.");
            }
        } catch (error) {
            ...
            <div className="p-4 rounded-xl border" style={{ backgroundColor: PRIMARY[50], borderColor: PRIMARY[200] }}>
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm flex items-center" style={{ color: PRIMARY[600] }}>
                            Lớp {student.className}
                        </p>
                    </div>
                </div>
            ...
            <div className="mb-6">
                ...
                <div className="flex gap-4">
                    <button
                        ...
                        className={`flex-1 py-3 rounded-xl text-sm font-medium transition-all text-center border-2 ${selectedOption === "vaccinated"
                            ? "text-white"
                            : "text-gray-700 border-gray-400"
                            }`}
                        style={{
                            backgroundColor: selectedOption === "vaccinated" ? SUCCESS[500] : GRAY[100],
                            borderColor: selectedOption === "vaccinated" ? SUCCESS[500] : BORDER.DEFAULT
                        }}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 py-3 rounded-xl text-sm font-medium transition-all text-center border-2 ${selectedOption === "notVaccinated"
                            ? "text-white"
                            : "text-gray-700 border-gray-400"
                            }`}
                        style={{
                            backgroundColor: selectedOption === "notVaccinated" ? ERROR[500] : GRAY[100],
                            borderColor: selectedOption === "notVaccinated" ? ERROR[500] : BORDER.DEFAULT
                        }}
                    >
